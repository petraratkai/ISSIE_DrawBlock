...
* getInputPortLocation, getOutputPortLocation 
    * highly inefficient, O(n) complexity, where n is the number of ports in the model
    * does not use regularity, almost all the code is shared between the two functions
* getOnePortLocation, getOnePortLocationNew
    * bad names, the older function has no reason to be kept
    * still O(n) complexity
* regex: terrible name, not descriptive at all
* getCompList
    * very long
...
* pasteSymbols
    * no need to sort the old symbol list, only need the maximum so could reduce complexity from O(n*logn) to O(n)
* getEquivalentCopiedPorts
    * checks every symbol in the model -> complexity could be improved
* update
    * very long, which makes it hard to read
...
* field names in Symbol could have been better, for example Symbol.Compo is not a very good name
